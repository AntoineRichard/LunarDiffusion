...
class GaussianDiffusion2D(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            in_channels (int): in/out image channels (C)
            in_dims (tuple): in/out image shape (H,W)
    ...
    def forward(self, x_0: Tensor, z_cond: Tensor = None, **kwargs) -> Tensor:
        ...
        assert (
            c == self.in_channels and h == self.in_dims[0] and w == self.in_dims[1]
        ), f"Expected input shape: [B, {self.in_channels}, {self.in_dims[0]}, {self.in_dims[1]}], but got {x_0.shape}"
    ...
    def sample(
        ...
        for t in tqdm(
            ...
            pred_noise = self.model(x_noisy, time=t_batch, z_cond=z_cond)
