...
class Sampler:
    ...
    def getDataset(self):
        ...
        return tf.data.Dataset.from_generator(
            ...
            output_shapes=(
                tf.TensorShape([self.hw, self.hw, 2]),
                tf.TensorShape([self.hw, self.hw, 1]),
            ),
        )
    ...
    def _getImg(self, key_dem, key_ort):
        ...
        raw_dem = raw_dem + random.random() * np.repeat(
            np.expand_dims(np.arange(self.hw, dtype=np.float32), -1), self.hw, -1
        ) / (self.hw / 2.0)
        raw_dem = raw_dem + random.random() * np.repeat(
            np.expand_dims(np.arange(self.hw, dtype=np.float32), 0), self.hw, 0
        ) / (self.hw / 2.0)
        raw_dem = (raw_dem * 1.0 - raw_dem.min()) / (raw_dem.max() - raw_dem.min())
        raw_dem = np.expand_dims(raw_dem - 0.5, -1)
        smt_dem = cv2.resize(
            cv2.resize(
                raw_dem, (self.hw // self.us, self.hw // self.us), cv2.INTER_AREA
            ),
            (self.hw, self.hw),
            cv2.INTER_CUBIC,
        )
